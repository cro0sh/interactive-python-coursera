...
class Sprite:
    ...
    def collide(self, other_object):
        if type(other_object) == set:
            for member in other_object:
                position = member.get_position()
                radius = member.get_radius()
                distance = dist(self.pos, position)
...
def draw(canvas):
    ...
    group_collide(rock_group, my_ship_thrust)
    ...
    if lives == 0: 
        started = False
        for member in rock_group:
            rock_group.remove(member)
            for memberm in missile_group:
                missile_group.remove(memberm)
        score = 0
        lives = 3
        soundtrack.rewind()
        soundtrack.pause()
...
def dist_rocks(a_object, group):
    for member in set(group):
        return dist(a_object.pos, member.pos) 
...
def group_group_collide(groupone, grouptwo):     
    countcollisions = 0
    grouptwo = set(grouptwo)
    for member in set(groupone):
         try:
            if group_collide(grouptwo, member) > 0:            
                groupone.remove(member)            
                member.update()                 
         except Exception, e:
            print e
         countcollisions += group_collide(groupone, grouptwo)      
    return countcollisions    

def process_sprite_group(canvas, setz):
    global score, lives
    for sprite in set(setz):        
        sprite.update()
        sprite.draw(canvas)  
        ...
        try:
            if group_collide(rock_group, missile_group) > 0:        
                setz.discard(sprite)
                score += 10
        ...
        if sprite.collide(my_ship) or sprite.collide(my_ship_thrust):
            setz.remove(sprite)
            lives -= 1
...
def rock_spawner():
    global rock_group
    dist_rock_constant = 150
    dist_rocks_ship = my_ship.get_radius() - dist_rock_constant <= dist_rocks(my_ship, rock_group) <= my_ship.get_radius() + dist_rock_constant

    if len(rock_group) < 12 and started and not dist_rocks_ship:
        rock_group.add(Sprite([random_number()[0], random_number()[1]], [random_float(-.2,.5), random_float(-.3,.6)], 0, random_float(-.1,.1), asteroid_image, asteroid_info))
